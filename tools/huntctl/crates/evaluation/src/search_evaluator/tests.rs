...
#[test]
fn worker_schedule_definition_rejects_lane_and_identity_tampering() {
    let schedule = EvaluationWorkerSchedule {
        schema: WORKER_SCHEDULE_SCHEMA.into(),
        worker_lanes: 2,
        planned_attempts: 3,
        assignments: vec![
            PlannedWorkerAssignment {
                trial_index: 0,
    ...
    validate_worker_schedule_definition(&schedule).unwrap();

    let mut wrong_lane = schedule.clone();
    wrong_lane.assignments[2].worker_id = "evaluation/worker-1".into();
    assert!(validate_worker_schedule_definition(&wrong_lane).is_err());
    ...
    assert!(validate_worker_schedule_definition(&duplicate_identity).is_err());
}

#[test]
fn completed_worker_claims_must_be_planned_exactly_once() {
    let schedule = EvaluationWorkerSchedule {
        schema: WORKER_SCHEDULE_SCHEMA.into(),
        worker_lanes: 2,
        planned_attempts: 2,
    ...
    validate_completed_worker_claims(
        &schedule,
        [("candidate-b", 1, "evaluation/worker-1")],
    )
    .unwrap();
    assert!(
        validate_completed_worker_claims(
            &schedule,
            [("candidate-a", 1, "evaluation/worker-1")],
        )
        .is_err()
    );
    assert!(
        validate_completed_worker_claims(
            &schedule,
            [("candidate-c", 1, "evaluation/worker-0")],
        )
        .is_err()
    );
    assert!(
