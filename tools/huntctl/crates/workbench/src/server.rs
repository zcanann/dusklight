    ...
    match read_http_request(stream) {
        Ok(request) => {
            ...
            match (request.method.as_str(), request.path.as_str()) {
                ...
                ("POST", "/api/tactic-route/start") => {
                    let result =
                        serde_json::from_slice::<BrowserTacticRouteStartRequest>(&request.body)
                            .map_err(|error| {
                                WorkbenchError::new(format!(
                                    "invalid tactic-route start request: {error}"
                                ))
                            })
                            .and_then(|start| start_tactic_route_learning(config, &start));
                    match result {
                        Ok(response) => json_response(&response).unwrap_or_else(|error| {
                            json_error(500, "Internal Server Error", &error.to_string())
                        }),
                ...
                ("POST", "/api/tactic-route/pause") => {
                    let result = serde_json::from_slice::<BrowserOptimizationLifecycleRequest>(
                        &request.body,
                    )
                    .map_err(|error| {
                        WorkbenchError::new(format!("invalid tactic-route pause request: {error}"))
                    })
                    .and_then(|pause| pause_tactic_route_learning(config, &pause));
                    match result {
                        Ok(response) => json_response(&response).unwrap_or_else(|error| {
                            json_error(500, "Internal Server Error", &error.to_string())
                        }),
                ...
                ("POST", "/api/tactic-route/replay") => {
                    let result =
                        serde_json::from_slice::<BrowserTacticRouteReplayRequest>(&request.body)
                            .map_err(|error| {
                                WorkbenchError::new(format!(
                                    "invalid tactic-route replay request: {error}"
                                ))
                            })
                            .and_then(|replay| replay_tactic_route_edge(config, &replay));
                    match result {
                        Ok(response) => json_response(&response).unwrap_or_else(|error| {
                            json_error(500, "Internal Server Error", &error.to_string())
                        }),
                ...
                ("POST", "/api/optimization/cancel") => {
                    ...
                    .map_err(|error| {
                        WorkbenchError::new(format!("invalid optimization cancel request: {error}"))
                    })
                    .and_then(|cancel| cancel_optimization_campaign(config, &cancel));
                    match result {
                        Ok(response) => json_response(&response).unwrap_or_else(|error| {
                            json_error(500, "Internal Server Error", &error.to_string())
                        }),
                ...
                ("POST", "/api/optimization/cleanup") => {
                    ...
                    .map_err(|error| {
                        WorkbenchError::new(format!(
                            "invalid optimization cleanup request: {error}"
                        ))
                    })
                    .and_then(|cleanup| cleanup_optimization_campaign(config, &cleanup));
                    match result {
                        Ok(response) => json_response(&response).unwrap_or_else(|error| {
                            json_error(500, "Internal Server Error", &error.to_string())
                        }),
                ...
                ("POST", "/api/optimization/candidate-detail") => {
                    let result =
                        serde_json::from_slice::<BrowserOptimizationCandidateDetailRequest>(
                            &request.body,
                        )
                        .map_err(|error| {
                            WorkbenchError::new(format!(
                                "invalid optimization candidate-detail request: {error}"
                            ))
                        })
                        .and_then(|detail| {
                            optimization_candidate_detail(
                                &config.repository_root,
                                &config.timeline_path,
                                &detail,
                            )
