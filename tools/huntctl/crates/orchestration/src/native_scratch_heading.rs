...
use dusklight_learning::scratch_action_catalog::{
    SCRATCH_ACTIONS_PER_HEADING, SCRATCH_HEADING_COUNT, scratch_action_catalog,
};
...
const CHECKPOINT_SCHEMA: &str = "dusklight-native-scratch-heading-checkpoint/v1";
...
impl ScratchHeadingSearch {
    pub fn new(incumbent_action_sequence: Vec<usize>) -> Result<Self, String> {
        if incumbent_action_sequence.is_empty()
            ...
            || incumbent_action_sequence
                .iter()
                .any(|action| *action >= SCRATCH_HEADING_COUNT * SCRATCH_ACTIONS_PER_HEADING)
        {
    ...
    pub fn validate(&self, seed: u64) -> Result<(), String> {
        let rebuilt = Self::new(self.incumbent_action_sequence.clone())?;
    ...
    pub fn next_candidate(&self, seed: u64) -> Result<Option<ScratchHeadingCandidate>, String> {
    ...
    fn candidates(&self, seed: u64) -> Result<Vec<ScratchHeadingCandidate>, String> {
        ...
        {
            let previous_heading_index = action / SCRATCH_ACTIONS_PER_HEADING;
            let variant_index = action % SCRATCH_ACTIONS_PER_HEADING;
            for replacement_heading_index in [
                (previous_heading_index + SCRATCH_HEADING_COUNT - 1) % SCRATCH_HEADING_COUNT,
                (previous_heading_index + 1) % SCRATCH_HEADING_COUNT,
            ] {
                let replacement_action =
                    replacement_heading_index * SCRATCH_ACTIONS_PER_HEADING + variant_index;
...
pub fn run_native_scratch_heading_refinement(
    ...
    let source = load_scratch_refinement_source(&config.scratch).map_err(heading_display)?;
    let root = config
        .scratch
        .repository_root
        .canonicalize()
    ...
    let prefix_frames = process_tape
        ...
        .to_vec();
    let catalog = scratch_action_catalog().map_err(heading_display)?;
    ...
    let mut checkpoint = if checkpoint_path.exists() {
        ...
        let search = ScratchHeadingSearch::new(source.incumbent_action_sequence.clone())
        ...
        let mut report = NativeScratchHeadingReport {
            ...
            candidates_remaining: search
                .remaining_candidates(source.seed)
    ...
    validate_heading_checkpoint(&checkpoint, config, &source, action_universe_sha256)?;
        ...
        && started.elapsed() < config.maximum_wall_time
    {
        let Some(candidate) = checkpoint
            .search
            .next_candidate(checkpoint.seed)
            .map_err(heading_error)?
        else {
            break;
        };
        ...
        checkpoint.report.candidates_remaining = checkpoint
            .search
            .remaining_candidates(checkpoint.seed)
            .map_err(heading_error)? as u64;
        checkpoint
            .report
            .attempts
...
mod tests {
    ...
    fn candidates_replace_only_heading_and_cover_both_neighbors() {
        let action = 3 * SCRATCH_ACTIONS_PER_HEADING + 7;
        let search = ScratchHeadingSearch::new(vec![action]).unwrap();
        let candidates = search.candidates(17).unwrap();
        ...
        assert!(candidates.iter().all(|candidate| {
            candidate.action_sequence[0] % SCRATCH_ACTIONS_PER_HEADING == 7
    ...
    fn headings_wrap_and_candidates_are_unique() {
        let search = ScratchHeadingSearch::new(vec![5, 5]).unwrap();
        let candidates = search.candidates(19).unwrap();
    ...
    fn progress_resumes_and_acceptance_resets_to_the_new_incumbent() {
        let mut search = ScratchHeadingSearch::new(vec![1, 18]).unwrap();
    ...
    fn binary_checkpoint_round_trips_and_rejects_corruption() {
        ...
        let mut report = NativeScratchHeadingReport {
            ...
            terminal_candidates: 0,
            strict_winners: 0,
            candidates_remaining: search.remaining_candidates(5).unwrap() as u64,
