...
pub const NATIVE_TACTIC_CAMPAIGN_SUMMARY_SCHEMA_V7: &str =
    "dusklight-native-tactic-campaign-summary/v7";
pub const NATIVE_TACTIC_CAMPAIGN_SUMMARY_SCHEMA_V8: &str =
    "dusklight-native-tactic-campaign-summary/v8";
...
pub struct NativeTacticCampaignOutcomeSummary {
    ...
    /// Seeds that locally evaluated at least one authenticated terminal
    /// proposal. This excludes support inherited through shared replay.
    #[serde(default, skip_serializing_if = "is_zero_u64")]
    pub terminal_producing_seeds: u64,
    /// Seeds with authenticated terminal support but no locally evaluated
    /// terminal proposal. This makes replay/demo inheritance explicit.
    #[serde(default, skip_serializing_if = "is_zero_u64")]
    pub terminal_support_only_seeds: u64,
...
pub struct NativeTacticCampaignCausalSummary {
    ...
    /// Supported pairs whose exact native ticks-to-terminal can be recovered
    /// for both lineages. These are the only pairs eligible for an outcome
    /// effect estimate.
    #[serde(default, skip_serializing_if = "is_zero_u64")]
    pub paired_terminal_return_exact_outcomes: u64,
    #[serde(default, skip_serializing_if = "is_zero_u64")]
    pub paired_terminal_return_policy_wins: u64,
    #[serde(default, skip_serializing_if = "is_zero_u64")]
    pub paired_terminal_return_control_wins: u64,
    #[serde(default, skip_serializing_if = "is_zero_u64")]
    pub paired_terminal_return_ties: u64,
    #[serde(default, skip_serializing_if = "is_zero_u64")]
    pub paired_terminal_return_policy_ticks_to_terminal: u64,
    #[serde(default, skip_serializing_if = "is_zero_u64")]
    pub paired_terminal_return_control_ticks_to_terminal: u64,
    #[serde(default, skip_serializing_if = "is_zero_u64")]
...
pub struct NativeTacticCampaignGoalReachabilitySummary {
    ...
    #[serde(default, skip_serializing_if = "is_zero_u64")]
    pub terminal_frontier_uncertainty_decisions: u64,
    /// Actual acquisition partitions used for action selection after any
    /// branch restore. This deliberately reports the branch clock rather than
    /// re-deriving a partition from decision cadence.
    #[serde(default, skip_serializing_if = "BTreeMap::is_empty")]
    pub selected_acquisition_rank_counts: BTreeMap<u64, u64>,
...
impl NativeTacticCampaignSummary {
    pub fn build(
        ...
        let terminal_support_only_seeds = route
            .terminal_seeds
            .checked_sub(terminal_producing_seeds)
            .ok_or_else(|| route_message("terminal-producing seeds exceed supported seeds"))?;
    ...
    pub fn validate(&self) -> Result<(), NativeTacticRouteRunError> {
        if !matches!(
            ...
            NATIVE_TACTIC_CAMPAIGN_SUMMARY_SCHEMA_V4
                ...
                | NATIVE_TACTIC_CAMPAIGN_SUMMARY_SCHEMA_V8
            ...
            || self.outcome.selected_terminal_seeds
                > if self.schema == NATIVE_TACTIC_CAMPAIGN_SUMMARY_SCHEMA_V8 {
                    self.outcome.terminal_producing_seeds
                } else {
            ...
            || !valid_terminal_outcome_counts(
                ...
                self.outcome.terminal_seeds,
                self.outcome.terminal_producing_seeds,
                self.outcome.terminal_support_only_seeds,
            ...
            || self.causal_chain.paired_terminal_return_exact_outcomes
                > self
                    .causal_chain
                    .paired_terminal_return_supported_comparisons
            || self.causal_chain.paired_terminal_return_exact_outcomes
                != self
                    .causal_chain
                    .paired_terminal_return_policy_wins
                    .saturating_add(self.causal_chain.paired_terminal_return_control_wins)
                    .saturating_add(self.causal_chain.paired_terminal_return_ties)
            || (self.causal_chain.paired_terminal_return_exact_outcomes == 0
                && (self
                    .causal_chain
                    .paired_terminal_return_policy_ticks_to_terminal
                    != 0
                    || self
                        .causal_chain
                        .paired_terminal_return_control_ticks_to_terminal
                        != 0))
            || (self.causal_chain.paired_terminal_return_exact_outcomes != 0
                && (self
                    .causal_chain
            ...
            || (matches!(
                self.schema.as_str(),
                NATIVE_TACTIC_CAMPAIGN_SUMMARY_SCHEMA_V7 | NATIVE_TACTIC_CAMPAIGN_SUMMARY_SCHEMA_V8
...
fn summary_schema_for_route(route_schema: &str) -> &'static str {
    match route_schema {
        NATIVE_TACTIC_ROUTE_REPORT_SCHEMA_V45 => NATIVE_TACTIC_CAMPAIGN_SUMMARY_SCHEMA_V8,
...
fn valid_terminal_outcome_counts(
    ...
    terminal_seeds: u64,
    terminal_producing_seeds: u64,
    terminal_support_only_seeds: u64,
    terminal_proposals: u64,
) -> bool {
    match schema {
        NATIVE_TACTIC_CAMPAIGN_SUMMARY_SCHEMA_V8 => {
            terminal_producing_seeds <= terminal_seeds
                && terminal_support_only_seeds <= terminal_seeds
                && terminal_producing_seeds.saturating_add(terminal_support_only_seeds)
                    == terminal_seeds
                && terminal_producing_seeds <= terminal_proposals
                && (terminal_producing_seeds == 0) == (terminal_proposals == 0)
        }
        NATIVE_TACTIC_CAMPAIGN_SUMMARY_SCHEMA_V6 | NATIVE_TACTIC_CAMPAIGN_SUMMARY_SCHEMA_V7 => {
            terminal_producing_seeds == 0
                && terminal_support_only_seeds == 0
...
fn terminal_outcome_counts(route: &NativeTacticRouteReport) -> (u64, u64, u64, u64) {
    ...
    let selected_terminal_decisions = route
        ...
        .count() as u64;
    let terminal_producing_seeds = route
        .seeds
        .iter()
        .filter(|seed| {
            seed.trace
    ...
    for decision in traces {
        ...
        if let Some(acquisition) = &decision.branch_acquisition {
                ...
                && acquisition.terminal_value_supported
            {
                ...
                if acquisition.maximum_ensemble_variance.is_some() {
                    terminal_frontier_uncertainty_decisions =
                        terminal_frontier_uncertainty_decisions.saturating_add(1);
                }
            }
...
fn causal_summary(route: &NativeTacticRouteReport) -> NativeTacticCampaignCausalSummary {
    ...
    let mut paired_terminal_ticks = BTreeMap::<Digest, [Option<u64>; 2]>::new();
    ...
    for decision in &traces {
        ...
        let terminal_ticks = paired_terminal_ticks
            .entry(paired.pair_sha256)
            .or_insert([None, None]);
        let mut exact_outcome_valid = true;
        if paired.control_first_step_terminal {
            exact_outcome_valid &= record_exact_terminal_ticks(
                &mut terminal_ticks[1],
                u64::from(paired.control_first_step_ticks),
                paired.incumbent_ticks_to_go,
            );
        }
        if decision.terminal {
            // `route_suffix_ticks` is the current root-derived route length
            // after the optimization source boundary. The pair source prefix
            // uses that same coordinate system.
            let ticks_to_terminal = decision
                .route_suffix_ticks
                .checked_sub(paired.source_prefix_ticks);
            let role_index = match paired.role {
                NativeTacticPairedTerminalReturnRole::Policy => 0,
                NativeTacticPairedTerminalReturnRole::Control => 1,
            };
            exact_outcome_valid &= ticks_to_terminal.is_some_and(|ticks| {
                record_exact_terminal_ticks(
                    &mut terminal_ticks[role_index],
                    ticks,
                    paired.incumbent_ticks_to_go,
                )
            });
        }
        if !exact_outcome_valid && invalid_paired_terminal_returns.insert(paired.pair_sha256) {
            paired_terminal_return_authority_violations =
                paired_terminal_return_authority_violations.saturating_add(1);
        }
    ...
    let exact_paired_terminal_returns = paired_latest
        .iter()
        .filter_map(|(pair_sha256, paired)| {
            (paired.status_after_decision == NativeTacticPairedTerminalReturnStatus::Complete
                && paired.policy_terminal_supported
                && paired.control_terminal_supported
                && !invalid_paired_terminal_returns.contains(pair_sha256))
            .then_some(paired_terminal_ticks.get(pair_sha256).copied())
            .flatten()
            .and_then(|[policy, control]| policy.zip(control))
        })
        .collect::<Vec<_>>();
    let paired_terminal_return_exact_outcomes = exact_paired_terminal_returns.len() as u64;
    let paired_terminal_return_policy_wins = exact_paired_terminal_returns
        .iter()
        .filter(|(policy, control)| policy < control)
        .count() as u64;
    let paired_terminal_return_control_wins = exact_paired_terminal_returns
        .iter()
        .filter(|(policy, control)| control < policy)
        .count() as u64;
    let paired_terminal_return_ties = exact_paired_terminal_returns
        .iter()
        .filter(|(policy, control)| policy == control)
        .count() as u64;
    ...
    let paired_terminal_return_policy_ticks_to_terminal = exact_paired_terminal_returns
        ...
        .map(|(policy, _)| *policy)
        .fold(0_u64, u64::saturating_add);
    let paired_terminal_return_control_ticks_to_terminal = exact_paired_terminal_returns
        .iter()
    ...
    NativeTacticCampaignCausalSummary {
        ...
        paired_terminal_return_policy_wins,
        paired_terminal_return_control_wins,
        paired_terminal_return_ties,
        paired_terminal_return_policy_ticks_to_terminal,
        paired_terminal_return_control_ticks_to_terminal,
        paired_terminal_return_authority_violations,
...
fn record_exact_terminal_ticks(slot: &mut Option<u64>, ticks: u64, budget: u64) -> bool {
    if ticks == 0 || ticks > budget {
        return false;
    }
    match slot {
        Some(recorded) => *recorded == ticks,
        None => {
            *slot = Some(ticks);
            true
        }
...
mod tests {
    ...
    fn v45_summary_requires_a_same_state_policy_effect() {
        ...
        route.seeds[0].trace[0].branch_acquisition = Some(TacticFrontierAcquisition {
            ...
            reward: -0.1,
            best_mean_q: Some(-12.0),
            best_goal_progress_per_tick: None,
            predicted_terminal_ticks_to_go: Some(12.0),
            predicted_total_terminal_ticks: Some(20.0),
            exact_terminal_ticks_to_go: None,
            exact_total_terminal_ticks: None,
            maximum_ensemble_variance: Some(0.25),
    ...
    fn terminal_outcomes_separate_discovery_from_retained_continuation() {
        ...
        route.seeds[0].trace[0]
            ...
            .unwrap()
            .terminal = true;
        assert_eq!(terminal_outcome_counts(&route), (1, 1, 0, 0));
        ...
        route.seeds[0].trace[0]
            ...
            .unwrap()
            .terminal = true;
        assert_eq!(terminal_outcome_counts(&route), (2, 1, 1, 1));
    }
    ...
    fn paired_terminal_returns_count_supported_and_censored_comparisons() {
        ...
        route.seeds[0].trace[0].source_route_ticks = 20;
        route.seeds[0].trace[0].route_suffix_ticks = 24;
        route.seeds[0].trace[0].terminal = true;
        route.seeds[0].trace[1].source_route_ticks = 21;
        route.seeds[0].trace[1].route_suffix_ticks = 26;
        route.seeds[0].trace[1].terminal = true;
        ...
        assert_eq!(supported.paired_terminal_return_control_wins, 0);
        assert_eq!(supported.paired_terminal_return_ties, 0);
        assert_eq!(supported.paired_terminal_return_policy_ticks_to_terminal, 4);
        assert_eq!(
            supported.paired_terminal_return_control_ticks_to_terminal,
            6
        );
    ...
    #[test]
    fn terminal_producing_seeds_are_distinct_from_inherited_support() {
