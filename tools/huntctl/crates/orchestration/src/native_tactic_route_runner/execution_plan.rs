...
mod tests {
    ...
    fn bounded_staleness_is_an_explicit_validated_execution_mode() {
        ...
        let mut cross_lane = request();
        cross_lane.replay_sharing = NativeTacticReplaySharingPlan::BoundedStaleness {
            maximum_stale_replay_revisions: 4,
        };
        let plan = NativeTacticExecutionPlan::build(cross_lane).unwrap();
        assert_eq!(plan.generations[0].lane_indices, vec![0, 1, 2, 3]);
    }
