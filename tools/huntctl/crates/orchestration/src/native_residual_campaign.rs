...
impl NativeResidualExecutionBinding {
    ...
    pub fn validate_files(
        ...
        if self.schema != NATIVE_RESIDUAL_EXECUTION_SCHEMA_V1
            ...
            || self.world_context_sha256 == Digest::ZERO
        ...
        {
            return Err(native_message(
                "native residual execution requires a 30 Hz process-boot tape through the full source horizon",
            ));
        }
    ...
    pub fn validate(
        ...
        let attempts_valid = self.attempts.iter().enumerate().all(|(index, attempt)| {
            attempt.repetition as usize == index + 1
                && attempt.worker_seed
                    == optimization.execution.deterministic_seeds
                        [index % optimization.execution.deterministic_seeds.len()]
...
mod tests {
    ...
    fn fixture() -> (
        ...
        .unwrap();
        let tape = InputTape {
            frames: vec![InputFrame::default(); required],
            ..InputTape::default()
        };
    ...
    fn execution_binding_seals_the_exact_native_checkpoint_authority() {
        let (root, _artifacts, optimization, executable, game_data, tape, program) = fixture();
        ...
        assert_eq!(report.source_frame, 500);
        assert_eq!(report.exploration_horizon_ticks, 160);
        assert_eq!(report.process_boot_tape_frames, 660);
