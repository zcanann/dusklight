...
fn print_usage() {
    ...
    eprintln!(
        "Learning-value matrix:\n  huntctl campaign materialize-learning-value-residual-cell --plan PLAN.json --base-request REQUEST.json --checkpoint ID --seed N --treatment independent-random-residual|cem-residual --output build/REQUEST.json [--repository-root DIR]\n  huntctl campaign materialize-learning-value-learning-cell --plan PLAN.json --base-request REQUEST.json --checkpoint ID --seed N --treatment demonstration-assisted-state-reactive|from-scratch-state-reactive|learned-then-residual-refinement --output build/REQUEST.json [--repository-root DIR]\n  huntctl campaign materialize-learning-value-loop-cell --plan PLAN.json --request REQUEST.json --execution EXECUTION.json --initial-corpus CORPUS.json --input EPISODES.dseps... --checkpoint ID --seed N --treatment demonstration-assisted-state-reactive|from-scratch-state-reactive|learned-then-residual-refinement --output build/LOOP.json [--repository-root DIR]\n  huntctl campaign materialize-learning-value-refinement-cell --plan PLAN.json --request LEARNING.json --loop-request LOOP.json --loop-state STATE.json --checkpoint ID --seed N --output build/REFINEMENT.json [--repository-root DIR]\n  huntctl campaign seal-learning-value-cell-evidence --plan PLAN.json --cell-root build/CELL --checkpoint ID --seed N --treatment KIND --output build/EVIDENCE.json [--repository-root DIR]\n"
    );
    eprintln!(
        "Objective campaigns:\n  huntctl conformance --suite SUITE.json --executable DUSKLIGHT --game-data GAME.iso --output build/DIR [--repository-root DIR] [--fidelity headless|unpaced-headful|realtime-headful]\n  huntctl campaign seal-learning-value-comparison-plan --input DRAFT.json --output build/PLAN.json [--repository-root DIR]\n  huntctl campaign validate-learning-value-comparison-plan --input PLAN.json [--repository-root DIR]\n  huntctl campaign seal-learning-value-cell-evidence --plan PLAN.json --input DRAFT.json --output build/CELL.json [--repository-root DIR]\n  huntctl campaign validate-learning-value-cell-evidence --plan PLAN.json --input CELL.json [--repository-root DIR]\n  huntctl campaign seal-learning-value-comparison-report --plan PLAN.json --cell CELL.json... --negative-control CONTROLS.json --output build/REPORT.json [--repository-root DIR]\n  huntctl campaign validate-learning-value-comparison-report --input REPORT.json [--repository-root DIR]\n  huntctl campaign seal-optimization-request --input DRAFT.json --output REQUEST.json [--repository-root DIR]\n  huntctl campaign validate-optimization-request --input REQUEST.json [--repository-root DIR]\n  huntctl campaign minimize-residual-winner --request REQUEST.json --execution EXECUTION.json --checkpoint CHECKPOINT.json --candidate CANDIDATE.json --output build/DIR [--candidate-budget N] [--repository-root DIR]\n  huntctl campaign validate-residual-winner-minimization --input SUMMARY.json [--repository-root DIR]\n  huntctl campaign prove-residual-winner-cold-replay --request REQUEST.json --execution EXECUTION.json --minimization SUMMARY.json --output build/DIR [--timeout-seconds N] [--repository-root DIR]\n  huntctl campaign validate-residual-winner-cold-replay --request REQUEST.json --execution EXECUTION.json --input build/DIR/proof.json [--repository-root DIR]\n  huntctl campaign seed-residual-reverse-curriculum --request REQUEST.json --id NEW_ID --initial-tail-ticks N --expansion-step-ticks N --minimum-successes N --minimum-behavior-classes N --minimum-success-millionths N --output CHILD.json [--repository-root DIR]\n  huntctl campaign expand-residual-reverse-curriculum --request REQUEST.json --execution EXECUTION.json --checkpoint CHECKPOINT.json --id NEW_ID --output CHILD.json [--repository-root DIR]\n  huntctl campaign tighten-residual-horizon --request PARENT.json --execution EXECUTION.json --checkpoint CHECKPOINT.json --id NEW_ID --proposed-horizon N --minimum-successes N --minimum-behavior-classes N --minimum-support-millionths N --output CHILD.json [--repository-root DIR]\n  huntctl campaign materialize-native-residual-execution --request OPTIMIZATION.json --game DUSKLIGHT --dvd GAME.iso --world-context WORLD.json --output build/DIR [--checkpoint-validation-ticks N] [--verify-state-hashes] [--repository-root DIR]\n  huntctl campaign run-residual-optimization --request OPTIMIZATION.json --execution EXECUTION.json [--repository-root DIR]\n  huntctl campaign run-residual-optimization --request OPTIMIZATION.json (--run-request HARNESS.json | --game DUSKLIGHT --dvd GAME.iso) [--output-run-request TEMPLATE.json] [--timeout-seconds N] [--repository-root DIR]\n  huntctl campaign seal-native-goal-learning-loop --request OPTIMIZATION.json --execution EXECUTION.json --initial-corpus CORPUS.json --input EPISODES.dseps... --output LOOP.json --journal-path PATH --state-path PATH --artifact-root PATH [--generation-limit N] [--rollouts-per-generation N] [--simulated-tick-budget N] [--repository-root DIR]\n  huntctl campaign validate-native-goal-learning-loop --loop-request LOOP.json --request OPTIMIZATION.json --execution EXECUTION.json [--repository-root DIR]\n  huntctl campaign status-native-goal-learning-loop --loop-request LOOP.json --request OPTIMIZATION.json --execution EXECUTION.json [--repository-root DIR]\n  huntctl campaign evaluate-native-goal-learning-checkpoints --loop-request LOOP.json --request OPTIMIZATION.json --execution EXECUTION.json --output build/REPORT.json [--repository-root DIR]\n  huntctl campaign run-native-goal-learning-loop --loop-request LOOP.json --request OPTIMIZATION.json --execution EXECUTION.json [--repository-root DIR]\n  huntctl campaign init-optimization-resume --request REQUEST.json [--repository-root DIR]\n  huntctl campaign status-optimization-resume --request REQUEST.json [--repository-root DIR]\n  huntctl campaign append-optimization-resume --request REQUEST.json --event EVENT.json [--repository-root DIR]\n  huntctl campaign --suite SUITE.json --case ID --output build/DIR --dry-run [--repository-root DIR] [--proposer scripted|random|structured|learned]...\n  huntctl campaign --suite SUITE.json --case ID --output build/DIR --run-request REQUEST.json --definition TOURNAMENT.json [--repository-root DIR] [--workers N]\n"
    );
    eprintln!(
    ...
    eprintln!(concat!(
        "\nNative search:\n",
        "  huntctl search evaluate-suffix-batch --request OPTIMIZATION.json --execution EXECUTION.json --input-tape PROCESS.tape --batch BATCH.json --output DIR [--repository-root DIR]\n",
        ...
        "  huntctl search continuous --method cem|cma-es --candidate SEED.json --axes AXES.json --output DIR (--run-request REQUEST.json [--repository-root DIR] | --game PATH --dvd PATH) [--anchored-prefix PREFIX.tape --milestones PROGRAM.dmsp --source-milestone NAME --source-boundary-fingerprint VALUE --goal-milestone NAME] [--generations N] [--population N] [--elites N] [--initial-sigma S] [--candidate-budget N] [--rng-seed N]\n",
    ...
    eprintln!(
        "  huntctl learn refine-scratch-headings --request OPTIMIZATION.json --execution EXECUTION.json --source build/SCRATCH --output build/DIR --seed N [--candidate-limit N] [--maximum-episode-ticks N] [--epsilon-per-million N] [--wall-time-seconds N] [--cold-replay-timeout-seconds N] [--repository-root DIR]"
    );
    eprintln!(
    ...
    eprintln!(
        "  huntctl learn refine-scratch-rolls --request OPTIMIZATION.json --execution EXECUTION.json --scratch-source build/SCRATCH --source build/HEADING --output build/DIR --seed N [--candidate-limit N] [--maximum-episode-ticks N] [--epsilon-per-million N] [--wall-time-seconds N] [--cold-replay-timeout-seconds N] [--repository-root DIR]"
    );
    ...
    eprintln!(
        "  huntctl learn tactic-route --request OPTIMIZATION.json --execution EXECUTION.json --output build/DIR [--plan PLAN.dtp | --seed N]... [--proposal-policy learned|frozen-policy|random-valid|structured-non-learning] [--value-treatment continuous_fitted_q_forest|goal_relabeled_fitted_q_knn|goal_relabeled_frontier_double_q|goal_relabeled_universal_frontier_double_q|local_generalized_fitted_q_knn] [--execution-strategy native-controller|progressive-audit] [--promoted-tactic-registry REGISTRY.dtmr] [--workers N] [--checkpoint-capacity-workers N] [--decisions-per-seed N] [--branch-every N] [--refit-every N] [--epsilon-per-million N] [--demonstration-chunk-ticks N] [--maximum-stale-replay-revisions N] [--memory-bytes N] [--wall-micros N] [--fault-inject before-dispatch|during-execution|after-native-completion|after-recovery-point-commit|after-decision-commit] [--fault-decision N] [--resume] [--repository-root DIR]\n  huntctl learn validate-tactic-campaign-completion --report ROUTE.json\n  huntctl learn project-tactic-route-accounting --report ROUTE.json --output ROUTE.json [--repository-root DIR]\n  huntctl learn project-tactic-campaign-summary --report ROUTE.json --plan PLAN.dtp --output SUMMARY.json\n  huntctl learn validate-tactic-campaign-summary --report ROUTE.json --plan PLAN.dtp --summary SUMMARY.json\n  huntctl learn prove-tactic-route-cold-replay --request OPTIMIZATION.json --execution EXECUTION.json --plan PLAN.dtp --report ROUTE.json --seed N --output EVIDENCE-DIR [--maximum-first-hit-tick N] [--repetitions 2] [--timeout-seconds 120] [--repository-root DIR]\n  huntctl learn validate-tactic-route-cold-replay --request OPTIMIZATION.json --execution EXECUTION.json --plan PLAN.dtp --report ROUTE.json --proof-root EVIDENCE-DIR [--repository-root DIR]\n  huntctl learn seal-tactic-cold-replay-bundle --scratch-bundle SCRATCH-BUNDLE --proof-root REPLAY-EVIDENCE --bundle BUNDLE-DIR\n  huntctl learn validate-tactic-cold-replay-bundle --bundle EVIDENCE-DIR\n  huntctl learn run-tactic-launch-smoke --request OPTIMIZATION.json --execution EXECUTION.json --output build/DIR --bundle EVIDENCE-DIR --seed N [--memory-bytes N] [--wall-micros N] [--repository-root DIR]\n  huntctl learn seal-tactic-launch-smoke --request OPTIMIZATION.json --execution EXECUTION.json --report ROUTE.json --bundle EVIDENCE-DIR [--repository-root DIR]\n  huntctl learn validate-tactic-launch-smoke --bundle EVIDENCE-DIR\n  huntctl learn tactic-throughput-curve --request OPTIMIZATION.json --execution EXECUTION.json --output build/DIR [--seed N] --decisions-per-seed N (minimum 16) --proposals-per-decision 16 --memory-bytes N [--repetitions 2] [--resume] [--stop-after-sample N] [--bundle EVIDENCE-DIR] [--repository-root DIR]\n  huntctl learn seal-tactic-throughput-curve --request OPTIMIZATION.json --execution EXECUTION.json --report THROUGHPUT.json --bundle EVIDENCE-DIR [--repository-root DIR]\n  huntctl learn validate-tactic-throughput-curve-bundle --bundle EVIDENCE-DIR\n  huntctl learn tactic-restore-locality --request OPTIMIZATION.json --execution EXECUTION.json --output build/DIR [--seed N] --workers N --decisions-per-seed N --proposals-per-decision N [--repetitions 2] [--repository-root DIR]\n  huntctl learn audit-tactic-fault-recovery --control-report ROUTE.json --recovered-report ROUTE.json --output AUDIT.json\n  huntctl learn seal-tactic-fault-recovery --request OPTIMIZATION.json --execution EXECUTION.json --control-report CONTROL/route-report.json --recovered-report RECOVERED/route-report.json --bundle EVIDENCE-DIR [--repository-root DIR]\n  huntctl learn validate-tactic-fault-recovery-bundle --bundle EVIDENCE-DIR\n  huntctl learn validate-tactic-scratch-discovery --request OPTIMIZATION.json --execution EXECUTION.json --report ROUTE-REPORT.json --output ACCEPTANCE.json --bundle BUNDLE-DIR [--repository-root DIR]\n  huntctl learn validate-tactic-scratch-bundle --bundle BUNDLE-DIR\n  huntctl learn audit-tactic-scratch-campaign --report ROUTE-REPORT.json --output AUDIT.json [--repository-root DIR]\n  huntctl learn audit-tactic-observations --request OPTIMIZATION.json --report ROUTE-REPORT.json --input CORPUS.dtqc [--input MORE.dtqc] --output AUDIT.json\n  huntctl learn audit-post-terminal-tactic-controls --report ROUTE-REPORT.json --output AUDIT.json [--repository-root DIR]\n  huntctl learn compare-tactic-scratch-campaigns --learned-report ROUTE.json --frozen-report ROUTE.json --random-report ROUTE.json --output COMPARISON.json [--repository-root DIR]\n  huntctl learn diagnose-tactic-terminal-routes --scratch-report ROUTE.json --demonstration-report DEMONSTRATION.json --demonstration-corpus CORPUS.dtqc --output DIAGNOSIS.json [--repository-root DIR]\n  huntctl learn validate-tactic-restore-locality --report REPORT.json"
    );
    eprintln!(
    ...
    eprintln!(
        "  huntctl learn promote-tactic-terminal-for-optimization --bundle EVIDENCE-DIR --output build/DIR [--request OPTIMIZATION.json --execution EXECUTION.json] [--id ID] [--workers N (retains first N deterministic seeds)] [--repository-root DIR]"
    );
    ...
    eprintln!(
        "\nOffline world geometry:\n  huntctl world boot-catalog --stage-root STAGE_ROOT --known-loader MAP_LOADER.h --output CATALOG.json [--artifact-store ROOT]\n  huntctl world inventory --stage-dir STAGE_DIR --stage STAGE_ID --output INVENTORY.json [--artifact-store ROOT]\n  huntctl world profile-catalog --input PROFILE-CATALOG.json [--artifact-store ROOT]\n  huntctl world context --game-data-sha256 SHA256 --inventory INVENTORY.json [--inventory MORE.json] --output CONTEXT.json [--artifact-store ROOT]\n  huntctl world spatial-index WORLD --output INDEX.json [--artifact-store ROOT]\n  huntctl world surface-graph WORLD --output GRAPH.json [--artifact-store ROOT]\n  huntctl world graph-query WORLD --room N --seed COLLISION_ID [--seed COLLISION_ID]... [--max-hops N] [--limit K]\n  huntctl world query point WORLD --room N --point X,Y,Z [--max-distance D] [--limit K] [FILTERS]\n  huntctl world query aabb WORLD --room N --min X,Y,Z --max X,Y,Z [--limit K] [FILTERS]\n  huntctl world query ray WORLD --room N --origin X,Y,Z --direction X,Y,Z --max-distance D [--limit K] [FILTERS]\n  WORLD: (--inventory INVENTORY.json | --stage-dir STAGE_DIR --stage STAGE_ID)\n  FILTERS: [--load-triggers-only] [--trigger-id ID] [--destination-stage ID] [--destination-room N] [--destination-point N]\n  huntctl world kcl --archive ROOM.arc --prism INDEX [--point X,Y,Z] [--kcl-name room.kcl] [--plc-name room.plc]\n  huntctl world kcl --kcl room.kcl --plc room.plc --prism INDEX [--point X,Y,Z]"
    );
    eprintln!(
        "\nStage observation survey:\n  huntctl survey init --catalog CATALOG.json --ledger LEDGER.json --game DUSKLIGHT --dvd GAME.iso [--probe neutral|movement|camera|targeting|basic-actions|contact-sweep|actor-activation|loading-sweep] [--probe-ticks N] [--timeout-ms N] [--attempts N] [--game-arg ARG]...\n  huntctl survey status --catalog CATALOG.json --ledger LEDGER.json\n  huntctl survey run --catalog CATALOG.json --ledger LEDGER.json --game DUSKLIGHT --dvd GAME.iso --state-root DIR [--limit N | --candidate ID]... [--workers N] [--game-arg ARG]...\n  huntctl survey actor-coverage --catalog CATALOG.json --ledger LEDGER.json --state-root DIR --output REPORT.json\n  huntctl survey observation-coverage --catalog CATALOG.json --ledger LEDGER.json --state-root DIR [--ledger LEDGER.json --state-root DIR]... --output REPORT.json\n  huntctl survey compact-artifacts --catalog CATALOG.json --ledger LEDGER.json --state-root DIR"
    );
}
...
fn mock_search_worker(args: &[String]) -> Result<(), Box<dyn Error>> {
    ...
    if let Some(card_root) = option(args, "--automation-card-root") {
        let card_root = Path::new(&card_root);
        if !card_root.is_dir() || fs::read_dir(card_root)?.next().is_some() {
            return Err("mock worker requires a fresh empty automation card root".into());
        }
        fs::write(card_root.join("mock-card-write.gci"), b"mock card state")?;
    }
