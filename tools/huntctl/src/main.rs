...
fn print_usage() {
    ...
    eprintln!(
        "  huntctl search golf-route-inputs --timeline FILE --segment TIMELINE_SEGMENT [--source-goal GOAL] [--goal GOAL] --output DIR (--run-request REQUEST.json [--repository-root DIR] | --game PATH --dvd PATH) [--candidate-budget N] [--workers N] [--repetitions N]"
    );
    eprintln!(
