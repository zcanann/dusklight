...
fn audit_return_restart_writers(args: &[String]) -> Result<(), Box<dyn Error>> {
...
fn validate_return_restart_audit(args: &[String]) -> Result<(), Box<dyn Error>> {
    let input = required_path(args, "--input")?;
    ...
    let manifest = FactPackManifest::build(
        ...
        vec![
            FactPackCoverage {
                ...
                scope: "orig".into(),
                status: CoverageStatus::Partial,
                detail: "DZS/DZR chunks, STAG message groups, indexed SCLS destinations, stage-level MULT room-background transforms, normal FILI metadata, and pointer-backed RTBL room-load tables are decoded; unresolved chunk semantics and physical reachability remain explicit.".into(),
            },
            FactPackCoverage {
                domain: CoverageDomain::ActorPlacements,
                scope: "orig".into(),
    ...
    let manifest = FactPackManifest::build(
        ...
        vec![
            FactPackCoverage {
                ...
                scope: "world".into(),
                status: CoverageStatus::Partial,
                detail: "Every planner-native SCLS, MULT room-background transform, normal FILI record, and RTBL room-load record is imported; SCLS remains an inert encoded destination unless a source-audited actor transition imports separately.".into(),
            },
            FactPackCoverage {
                domain: CoverageDomain::ActorPlacements,
                scope: "world".into(),
            ...
            FactPackCoverage {
                ...
                scope: "world".into(),
                status: CoverageStatus::Unavailable,
                detail: "The planner-native inventory-set v3 does not decode KCL/PLC, spatial indexes, or collision/SCLS joins.".into(),
            },
            FactPackCoverage {
                domain: CoverageDomain::PhysicalFeasibility,
                scope: "world".into(),
