...
For exact GZ2E01 L1/L5 boss doors, v14 also imports the actor-local `checkArea`
...
For the exact D_MN07 room-6 `dr` placement, v14 separately imports the two
...
For every exact F_SP109 room-0 `R_Gate` layer, v14 keeps three causal states
...
fully open without consuming a key or writing that switch. The F_SP121 copies
remain excluded because switches `0x81`/`0x82` are outside the modeled memory-
switch domain.

For the exact F_SP118 room-1 and room-2 `CrvGate` parents, v14 imports normal
...
- Continue importing the audited F_SP121 rider/external-switch boundaries only
  after their switch-domain and external-writer state can be represented. The
  exact GZ2E01 keyed-family census and current v14 import
