...
| `cutscene-corruption-hypothesis/v1` | Exact-context unknown-evidence producer whose sole effect is the named all-STB-lookups-missing field, with explicit failure-site, predicate, and completed-prefix unknowns | Evidence that actor corruption really produces that failure, runtime dispatch flags, location, return-place writes, or savewarp |
