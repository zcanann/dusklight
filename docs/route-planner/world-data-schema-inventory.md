...
`1780d0f1e3a7fb4d0fb9ad94ef79446d7958da08d26d871f34d5f3d7c64fa41f`.
...
| `extracted-orig-bundle/v5` | Source manifest; separate generic actor/scaled/door, treasure (`TRES`/`TRE*`), and player-spawn (`PLYR`) placements with raw records; STAG message group; indexed SCLS destinations; stage-level `MULT` room-background transforms; normal stage/room `FILI` metadata; REVT event/exit coordinates; LBNK demo-archive selections; decoded numbered BMG flow graphs; explicit ignored message candidates | KCL/PLC, most other DZS/DZR chunk bodies, field-map-only `FILI` layout, actor/treasure parameter meaning, JStudio cutscene internals, message text, runtime bindings |
| `extracted-orig-world-inventories/v2` | Planner-native, content-addressed stage grouping of every decoded DZS/DZR source, chunk directory, actor/treasure placement, player spawn, raw/normalized SCLS record, `MULT` room transform, and normal `FILI` record; explicit domain coverage | KCL/PLC and collision joins are explicitly `unavailable`; actor-specific semantics and physical reachability are not inferred |
...
not acquire one. `extracted-orig-bundle/v5` and
`extracted-orig-world-inventories/v2` are planner-native. The latter is built
directly from the former by `orig_world.rs`; it never imports Huntctl types or
artifacts. World facts v18 consume the native set directly. They represent its
...
- 6,128 DZS/DZR chunk entries with their tag, count, and offset retained; v5
...
V5 retains the placement-layout parity work from v4 and adds source-audited
`MULT` and normal `FILI` records. `PLYR` appears in 304
...
`RCAM`, field-map-only `FILI`, `LGT*`, `RPAT`, `RPPN`, `RTBL`, and
...
planner-native v5 extraction produce 95 actor placements, five player spawns,
...
For exact GZ2E01 L1/L5 boss doors, v18 also imports the actor-local `checkArea`
...
For the exact D_MN07 room-6 `dr` placement, v18 separately imports the two
...
For every exact F_SP109 room-0 `R_Gate` layer, v18 keeps three causal states
...
fully open without consuming a key or writing that switch. V17 applies the same
...
For the exact R_SP116 room-6 pair, v18 imports the one-shot `Wchain` producer and
...
For the exact F_SP118 room-1 and room-2 `CrvGate` parents, v18 imports normal
...
- Add planner-owned KCL/PLC decoding and spatial artifacts to enrich native
  facts; v18 already imports the non-spatial bundle domains directly and marks
...
- Continue importing other source-censused actor writers and consumers against
  their exact state domains. V18 already composes the R_SP116 `Wchain` producer
