...
| `message-flow-entry-contract-set/v3` and `compiled-message-flow-entry-set/v3` | Exact compiled-flow schema/digest, stage/message resource joins, optional raw actor placement identity, resolved flow label, authored guards, obligations, unknowns, speaker context, source-pinned presentation requests, and deterministic entry mechanics | Unauthored callers, presentation-actor success/grant, inferred interaction geometry, actor behavior not established by placement, or equivalence across builds/languages |
