...
| `kshtr00` / `L3Bdoor` key shutters | Runtime type, event selector, check-key bit, switch, placement | Small-key or boss-key guard plus memory switch | Event phase, queued decrement, animation and background collision | Set switch reconstructs the shutter open with collision released; the separate `vshuter` consumer opens from current-room one-zone switch `0xef` written by the colocated one-shot wolf chain |
| `Wchain` | Switch, repeat nibble, pose | Current-room one-zone switch `0xef`; authored repeat `0xf` normalizes to one-shot | Wolf attention, ready jump, ride/tension timer, pull length, `mNowSwitch`, actor execute | A set switch reconstructs this one-shot chain pulled down; its colocated `vshuter` observes the same switch |
