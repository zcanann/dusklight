...
void ImGuiMenuTools::draw() {
    ...
    if (ImGui::BeginMenu("Debug")) {
        ...
        if (ImGui::BeginMenu("Collision View")) {
            ...
            ImGui::Indent();
            ImGui::Checkbox(
                "Highlight near-vertical angles", &collisionView.colorNearVerticalWalls);
            ...
            ImGui::SliderFloat(
                "Capture range around Link", &collisionView.drawRange, 0.0f, 5000.0f, "%.0f units");
            ...
            ImGui::DragFloat("Upward extent", &collisionView.ceilingExtentUp, 1.0f, 0.0f, 10000.0f,
            ...
            ImGui::SliderFloat(
                "Opacity##colliders", &collisionView.colliderViewOpacity, 0.0f, 100.0f);
            ImGui::EndMenu();
        }
        ...
        auto& eponaBlockerView = getTransientSettings().eponaBlockerView;
        if (ImGui::BeginMenu("Epona Blocker View")) {
            ImGui::Checkbox(
                "HorseNoEntry collision polygons", &eponaBlockerView.showCollisionPolygons);
            ImGui::Checkbox(
                "Horse wall-code polygons", &eponaBlockerView.showHorseWallPolygons);
            ImGui::Checkbox("Horse-stop volumes", &eponaBlockerView.showStopVolumes);
            ImGui::BeginDisabled(!eponaBlockerView.showStopVolumes);
            ImGui::Indent();
            ImGui::Checkbox("Show inactive volumes", &eponaBlockerView.showInactiveStopVolumes);
            ImGui::Unindent();
            ImGui::EndDisabled();
            ImGui::Checkbox("Wireframe only##epona-blockers", &eponaBlockerView.wireframeOnly);
            ImGui::SliderFloat("Opacity##epona-blockers", &eponaBlockerView.opacity, 0.0f, 100.0f);
            ImGui::SliderFloat(
                "Draw Range##epona-blockers", &eponaBlockerView.drawRange, 100.0f, 5000.0f);
            ImGui::TextDisabled("HorseNoEntry polygons: orange");
            ImGui::TextDisabled("Horse-only walls: cyan");
            ImGui::TextDisabled("Conditional horse walls: yellow");
            ImGui::TextDisabled("Active horse-stop volumes: magenta");
            ImGui::TextDisabled("Inactive horse-stop volumes: dim purple");
            ImGui::TextDisabled("Volumes include the gameplay vertical margins.");
            ImGui::EndMenu();
        }
        ...
        ImGui::MenuItem(
            "Process Management", hotkeys::SHOW_PROCESS_MANAGEMENT, &m_showProcessManagement);
...
void ImGuiMenuTools::ShowPlayerInfo() {
    ...
    if (ImGui::Begin("Player Info", nullptr, windowFlags)) {
        ...
        double speedXzy = 0.0;
        if (player != nullptr) {
            speedXzy = sqrtf(player->speed.x * player->speed.x + player->speed.z * player->speed.z +
        ...
        ImGui::Separator();
        ImGui::Text("Link");
        ImGuiStringViewText(player != nullptr ? fmt::format("Position: {: .4f}, {: .4f}, {: .4f}\n",
        ...
        ImGuiStringViewText(
            player != nullptr ? fmt::format("Speed (3D): {: .4f}\n", speedXzy) : "Speed (3D): ?\n");

        ImGuiStringViewText(
            player != nullptr ? fmt::format("Angle: {0}\n", player->shape_angle.y) : "Angle: ?\n");

        ImGuiStringViewText(
            player != nullptr ? fmt::format("Room: {0}\n", fopAcM_GetRoomNo(player)) : "Room: ?\n");
        ...
        ImGui::Separator();
        ImGui::Text("Epona");
        ImGuiStringViewText(horse != nullptr ? fmt::format("Position: {: .4f}, {: .4f}, {: .4f}\n",
                                                   horse->current.pos.x, horse->current.pos.y,
                                                   horse->current.pos.z) :
                                               "Position: ?, ?, ?\n");

        ImGuiStringViewText(horse != nullptr ?
                                fmt::format("Velocity (XYZ): {: .4f}, {: .4f}, {: .4f}\n",
        ...
        ImGuiStringViewText(
            horse != nullptr ? fmt::format("Angle: {0}\n", horse->shape_angle.y) : "Angle: ?\n");

        ImGuiStringViewText(
            horse != nullptr ? fmt::format("Room: {0}\n", fopAcM_GetRoomNo(horse)) : "Room: ?\n");
